...
pub fn post_response<'a, D: Encodable<Encoder<'a>, io::IoError>,
                 U: Decodable<Decoder, DecoderError>>(req: Request, res: Response, handler: |U| -> Result<D, HTTPError>) -> Result<(), &'static str> {
    ...
    match u.and_then(handler) {
        ...
        Err(HTTPError{status: st, error: er}) =>
            write_out(er.as_slice(), st, res)
    }
 }

fn read_json_post<T: Decodable<Decoder, DecoderError>>(mut req : Request) -> io::IoResult<T> {
